//Ecuación objetivo
var goal = {squared: 0, lineal: 0, constant: 0};
//Solución
var solution = {xLeft: 0, uLeft: 0, xRight: 0, uRight: 0};
...
function init(s, l, c, xL, uL, xR, uR) {
    goal.squared = s;
    goal.lineal = l;
    goal.constant = c;
    solution.xLeft = xL;
...
function areEquivalent(eq1, eq2) {
    return      eq1.squared == eq2.squared
            &&  eq1.lineal == eq2.lineal
...
function isSolved(eq) {
    return 	areEquivalent(goal, eq)
            &&  eq.xLeft == solution.xLeft
            &&  eq.uLeft == solution.uLeft
            &&  eq.xRight == solution.xRight
            &&  eq.uRight == solution.uRight;
}
...
function equationToString(eq) {
    var finalExpr = cuadraticExpressionToString(eq.squared, eq.lineal, eq.constant);
...
function updateCurrentEquation() {
    ...
    if (step == 0) {
        ...
        if (areEquivalent(current, goal)) {
            mensaje.innerHTML = "<span style='color: green'>&iexcl;Genial! ahora factoriza la ecuaci&oacute;n</span>";
            step = 1;
        }
    } else if (step == 1) {
        if (isSolved(current)) {
